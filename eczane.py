...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .arama-kutusu { 
            width: 100%; max-width: 700px; margin: 0 auto 30px;
            background: #fff; border-radius: 50px; padding: 5px;
            box-shadow: 0 4px 20px rgba(0,0,0,0.1);
            display: flex;
            transition: 0.3s;
        }
        .arama-kutusu:focus-within { box-shadow: 0 4px 30px rgba(59,130,246,0.3); }
        .arama-kutusu input { 
        ...
        .bos-mesaj i { font-size: 4em; display: block; margin-bottom: 20px; opacity: 0.3; }
        .bos-mesaj h3 { font-size: 1.5em; color: #1e293b; margin-bottom: 10px; }
        .footer { text-align: center; padding: 30px 20px; color: #9ca3af; font-size: 0.8em; line-height: 1.8; }
        .footer .acil { color: #dc2626; font-weight: 700; }
        .yukleniyor { text-align: center; padding: 40px; }
        .spinner { 
        ...
        .hata { text-align: center; padding: 40px; color: #dc2626; }
        .hata i { font-size: 3em; display: block; margin-bottom: 15px; }
...
<body>
    <div class="header">
        ...
        <div class="subtitle">🔍 İl veya ilçe adı yazın</div>
    </div>
    <div class="container">
        ...
        <div class="sonuclar" id="sonuclar">
            <div class="bos-mesaj">
                ...
                <p>Yukarıdaki arama kutusuna il veya ilçe adı yazın.</p>
                <p style="font-size: 0.85em; margin-top: 10px; color: #cbd5e1;">Örnek: İstanbul, Kadıköy, Ankara, Çankaya, İzmir, Mardin...</p>
            </div>
        </div>
    </div>
    <div class="footer">
...
def index():
    from datetime import datetime
    return render_template_string(INDEX_HTML, now=datetime.now().strftime("%d.%m.%Y %H:%M"))
...
def ara():
    ...
    # İlçe olarak dene - yaygın iller
    yaygin_iller = ["İstanbul", "Ankara", "İzmir", "Bursa", "Antalya", "Adana", "Konya", "Gaziantep", "Mersin", "Kocaeli", "Mardin", "Diyarbakır", "Erzurum", "Samsun", "Eskişehir"]
    ...
    <div class="bos-mesaj">
        ...
        <p style="font-size: 0.85em; margin-top: 10px; color: #cbd5e1;">
            İpucu: İstanbul, Kadıköy, Ankara, Çankaya, İzmir, Mardin...
        </p>
    </div>
    '''
...
def sonuc_olustur(veri):
    ...
    <div style="padding: 10px 0;">
        <div style="display: flex; align-items: center; gap: 12px; padding: 16px 20px; background: #f8fafc; border-radius: 12px; margin-bottom: 20px; flex-wrap: wrap;">
            <div style="width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; background: #3B82F6; color: #fff; font-weight: 700;">
                <i class="fas fa-map-marker-alt"></i>
            </div>
            <div style="flex: 1; min-width: 150px;">
                <div style="font-size: 0.85em; color: #64748b;">{veri['il']}</div>
                <div style="font-size: 1.3em; font-weight: 700; color: #1e293b;">{veri['ilce']}</div>
            </div>
            <div style="background: #dc2626; color: #fff; padding: 6px 16px; border-radius: 20px; font-weight: 600; font-size: 0.9em;">
                {len(veri['eczaneler'])} Nöbetçi Eczane
            </div>
        </div>
    '''
    ...
    for eczane in veri["eczaneler"]:
        ...
        html += f'''
        <div style="border: 1px solid #e2e8f0; border-radius: 12px; padding: 18px 22px; margin-bottom: 12px; transition: 0.2s;">
            <div style="font-size: 1.1em; font-weight: 700; color: #1e293b; margin-bottom: 8px; display: flex; align-items: center; gap: 8px;">
                <span style="width: 8px; height: 8px; border-radius: 50%; background: {renk}; flex-shrink: 0;"></span>
                {eczane['isim']}
            </div>
        '''
        if eczane.get('adres'):
            html += f'<div style="color: #4b5563; font-size: 0.9em; margin-bottom: 6px; display: flex; align-items: flex-start; gap: 10px; line-height: 1.5;"><i class="fas fa-map-marker-alt" style="color: #64748b; width: 16px; margin-top: 2px;"></i> {eczane["adres"]}</div>'
        if eczane.get('tarif'):
            html += f'<div style="background: #fffbeb; border-left: 3px solid #f59e0b; padding: 10px 14px; border-radius: 0 8px 8px 0; margin: 10px 0; color: #92400e; font-size: 0.88em; line-height: 1.5;">🧭 <strong>Tarif:</strong> {eczane["tarif"]}</div>'
        html += '<div style="display: flex; gap: 8px; flex-wrap: wrap; margin-top: 10px;">'
        if eczane.get('telefon'):
            telefon = eczane['telefon'].replace(' ', '').replace('-', '')
            html += f'<a href="tel:{telefon}" style="padding: 8px 18px; border-radius: 20px; font-weight: 600; font-size: 0.82em; text-decoration: none; background: #dcfce7; color: #166534; display: inline-flex; align-items: center; gap: 6px; transition: 0.2s;"><i class="fas fa-phone-alt"></i> Ara</a>'
        if eczane.get('maps_url'):
            html += f'<a href="{eczane["maps_url"]}" target="_blank" style="padding: 8px 18px; border-radius: 20px; font-weight: 600; font-size: 0.82em; text-decoration: none; background: #dbeafe; color: #1e40af; display: inline-flex; align-items: center; gap: 6px; transition: 0.2s;"><i class="fas fa-map-marked-alt"></i> Harita</a>'
        html += '</div></div>'
    
    return html

# Vercel için entry point
app.debug = False
